...
from .argoverse_detection import ArgoverseDetection
...
class ArgoverseDataset(ArgoverseDetection):
    def __init__(
        self, ann_file, root, remove_images_without_annotations, transforms=None, proposal_file=None
    ):
        super(ArgoverseDataset, self).__init__(root, ann_file)
        ...
        self.json_category_id_to_contiguous_id = {
            self.class_mapping[v]: i + 1 for i, v in enumerate(self.coco.getCatIds())
        }
    ...
    def __getitem__(self, idx):
        img, anno = super(ArgoverseDataset, self).__getitem__(idx)
        ...
        if anno == [] and 'unlabeled' in self.ann_file:
            ...
            classes = [self.class_mapping[obj["category_id"]] for obj in anno]
            # print(classes)
